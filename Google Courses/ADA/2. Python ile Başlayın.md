...
Python'da küme oluşturmak için iki fonksiyon vardır: set() ve frozenset(). Bunları herhangi bir yinelenebilir nesne üzerinde kullanın. Veya boş kümeler oluşturmak için bu işlevleri kullanın.

### [**set()**](https://docs.python.org/3/library/stdtypes.html#set-types-set-frozenset)

-   Bu, değiştirilebilir bir veri türüdür.
    
-   Değiştirilebilir olduğu için, bu sınıf [kümeye veri eklemek ve kümeden veri çıkarmak içinek yöntemlerle](https://docs.python.org/3/library/stdtypes.html#frozenset.update) birlikte gelir.
    
-   Herhangi bir yinelenebilir nesneye uygulanabilir ve bu nesneden yinelenen öğeleri kaldırır.
    
-   Sırasızdır ve indekslenemez. 
    
-   Bir kümedeki öğeler hashlenebilir olmalıdır; genellikle bu, değişmez olmaları gerektiği anlamına gelir. (Hashing hakkında daha fazla bilgi için ek kaynaklara bakın).
    

Aşağıdaki örneklerde, çeşitli veri türleri kullanılarak dört set örneklendirilmiştir:
...
Yukarıdaki örnekte, biri tamsayı diğeri float olmasına rağmen 2 ve 2.0 'ın eşdeğer olarak değerlendirildiğine dikkat edin. 
...
Yukarıdaki örnekte, (1, 2, 2, 2, 3) bir tuple olup hashlenebilir (≈ değişmez) ve dolayısıyla sonuç kümesinde ayrı bir tek eleman olarak ele alınır.
...
Yukarıdaki örnek bir hata verir çünkü bir kümenin her bir öğesi hashlenebilir (≈ değişmez) olmalıdır, ancak {'a', 'b', 'c'} değişebilir (hashlenemez) bir nesne olan bir kümedir.

Aşağıdaki örnekte, setler için kullanılabilen ancak frozensetler için kullanılamayan özel yöntemlerden biri olan add() yöntemi gösterilmektedir.
...
# {'mother', 'hamster', 'elderberries', 'father'}
```

example_d kümesine bir öğe eklendi ve böylece değiştirildi. Bu, set sınıfının değişebilirliğine bir örnektir.

### [frozenset()](https://docs.python.org/3/library/stdtypes.html#frozenset)

Frozensetler Python'daki bir başka küme türüdür. Bunlar kendi sınıflarıdır ve değişmez olmaları dışında kümelere çok benzerler.

-   Bu değişmez bir veri türüdür.
    
-   Herhangi bir yinelenebilir nesneye uygulanabilir ve bu nesneden yinelenen öğeleri kaldırır.
    
-   Değişmez oldukları için frozensetler sözlük anahtarları olarak ve diğer kümelerde öğe olarak kullanılabilir.
    

Bu örnekte, bir set içinde bir frozenset kullanılmaktadır.
...
Daha önceki example_c 'den farklı olarak, bu set hata vermez. Bunun nedeni, değişmez bir tür olan ve bu nedenle kümelerde kullanılabilen bir frozenset içermesidir.

## Küme yöntemleri

Kümeler, bir veri yapısında hangi değerlerin bulunduğunu belirlemek ve yinelenen değerleri ortadan kaldırmak için kullanışlıdır. Kümelerle çalışmaya işlevsellik ve güç katan kesişim, birleşim, fark ve simetrik fark gibi çok sayıda küme yöntemi vardır.
...
-   Bu işlev için işleç boru ( | ) şeklindedir.
...
-   Bu işlevin işleci ve işaretidir (&).
...
### [fark()](https://docs.python.org/3/library/stdtypes.html#frozenset.difference) 

-   Küme içinde diğerlerinde olmayan elemanlar içeren yeni bir küme döndürür.
    
-   Bu fonksiyon için operatör çıkarma operatörüdür ( - ).
...
-   Elemanları kümede veya diğerinde olan, ancak her ikisinde de olmayan yeni bir küme döndürür.
    
-   Bu işlev için işleç şapkadır ( ^ ).
...
-   Mevcut sınıf yöntemlerinin tam listesi de dahil olmak üzere [kümeler ve frozensetler](https://docs.python.org/3/library/stdtypes.html#set-types-set-frozenset) hakkında daha fazla bilgi için Python belgelerine bakın.
    
-   Kümelere özgü (ve frozenset'lerde kullanılamayan) yöntemler için bu [Python set yöntemleri belgesine](https://docs.python.org/3/library/stdtypes.html#frozenset.update) bakın.
    
-   Kümelerle ilgili daha fazla örnek için, kümelerle ilgili [Python](https://docs.python.org/3/tutorial/datastructures.html#sets) eğitimine bakın.
    
-   Hash tabloları, bir şeyi hashlenebilir yapan şey ve bir kavram olarak hashleme hakkında daha fazla bilgi için [Runestone Academy](https://runestone.academy/ns/books/published/pythonds/SortSearch/Hashing.html)'nin bu [titletitlekaynağına](https://runestone.academy/ns/books/published/pythonds/SortSearch/Hashing.html) bakın. Orijinal hashing algoritmalarının doğuşu hakkında ilginç bir hikaye için bu [IEEE](https://spectrum.ieee.org/hans-peter-luhn-and-the-birth-of-the-hashing-algorithm) [Spectrum makalesine](https://spectrum.ieee.org/hans-peter-luhn-and-the-birth-of-the-hashing-algorithm) göz atın.

# Python kütüphanelerini, paketlerini ve modüllerini anlama

Son zamanlarda Python kütüphaneleri, paketleri ve modülleri hakkında bilgi edindiniz. Gördüğünüz gibi, bu araçları içe aktarmak veri profesyonellerinin zaman kazanmasını sağlar ve programlamalarını geliştirir. Yaygın olarak kullanılan kütüphanelerin bir diğer avantajı da yetenekli ve bilgili programcılar tarafından sürekli gözden geçirilip güncellenmeleridir. Bu nedenle, bu kütüphanelerin temelinde yer alan kodun yüksek kalitede olduğundan emin olabilirsiniz.

Bu yazıda, kütüphanelerin, paketlerin ve modüllerin temel özelliklerini, bunların birbiriyle nasıl ilişkili olduğunu ve bir veri profesyoneli olarak kullanabileceğiniz temel modülleri daha ayrıntılı öğreneceksiniz.

## **Kütüphaneler, paketler ve modüller**

Bir **kütüphane**, yeniden kullanılabilir kod modüllerinin ve onlara eşlik eden belgelerin bütünüdür. Kütüphaneler, **paketler** içinde sunulur ve ihtiyaç duyduğunuzda kod ortamınıza aktarabileceğiniz şekilde kurulurlar. Genellikle “kütüphane” ve “paket” terimleri birbirinin yerine kullanılır. Bu eğitim programında da genellikle her ikisine birden “kütüphane” denilecektir, ancak her iki terime de aşina olmanız önemlidir.

**Modüller**, kütüphanelere benzer şekilde, ilişkili sınıflar ve fonksiyonlardan oluşan gruplardır; ancak genellikle kütüphanelerin alt bileşenleridir. Yani, bir kütüphane birçok farklı modüle sahip olabilir ve siz isterseniz tüm kütüphaneyi, isterseniz sadece ihtiyacınız olan modülü içe aktarabilirsiniz.

## **Import (İçe Aktarma) İfadeleri**

Python’un standart kütüphanesi dışındaki kütüphaneler ve modüller, genellikle ihtiyaca göre çalışma ortamınıza içe aktarılmalıdır. Önce bu ek kütüphaneleri kurmanız, ardından ihtiyacınıza göre içe aktarmanız gerekir.

Bir kütüphane veya modülü içe aktarmak için bir `import` ifadesi kullanılır. `import` ifadeleri belirli bir sözdizimi gerektirir ve **import** anahtar kelimesiyle yazılır. İşte bazı örnekler:

```python
import numpy
```

Bu ifade, NumPy kütüphanesini çalışma ortamınıza aktarır. Bu komutu çalıştırdıktan sonra, NumPy’nin tüm sınıf ve fonksiyonlarına erişebilirsiniz. Örneğin, `[2, 4, 6]` üzerinde `array()` fonksiyonunu kullanmak isterseniz şunu yazarsınız:

```python
numpy.array([2, 4, 6])
```

Burada `array()` fonksiyonuna ulaşmak için önüne `numpy` yazmanız gerekir, çünkü bu fonksiyon NumPy kütüphanesinden gelmektedir.

### Takma Ad (Aliasing)

Python kütüphanelerinde zaman kazandıran bir başka özellik ise **takma ad** (aliasing) kullanımıdır. Böylece bir kütüphanenin tam adını her seferinde yazmak zorunda kalmazsınız. Bunun yerine **as** anahtar kelimesiyle bir takma ad tanımlarsınız:

```python
import numpy as np
```

Bu durumda NumPy kütüphanesi `np` takma adıyla içe aktarılır. İstediğiniz herhangi bir takma adı kullanabilirsiniz; ancak yaygın olarak kullanılan kütüphanelerin alışılmış takma adları vardır. Bu takma adlardan sapmak, kodunuzu başkalarıyla paylaştığınızda kafa karışıklığına yol açabilir. İşte veri profesyonelleri tarafından sıkça kullanılan bazı kütüphaneler ve takma adları:
...
* **NumPy**: Vektör ve matris işlemleri için kullanılır.
* **Pandas**: Tablo verilerinin işlenmesi ve analizinde kullanılır.
* **Seaborn** ve **matplotlib**: Grafik, çizelge ve diğer görselleştirme işlemleri için kullanılır.

Bu içe aktarmaları yaptıktan sonra, ilgili fonksiyonu kullanırken takma adı fonksiyon adının önüne eklersiniz. Örneğin, `array()` fonksiyonunu takma adla şöyle kullanabilirsiniz:

```python
np.array([2, 4, 6])
```

### Ek İçe Aktarma Sözdizimi

#### **Modül İçe Aktarma**

Daha önceki örneği hatırlayalım:

```python
import matplotlib.pyplot as plt
```

Bu sözdizimi diğerlerinden biraz farklıdır. Burada `matplotlib` kütüphane, `pyplot` ise içindeki bir modüldür. `pyplot` modülü `plt` takma adıyla içe aktarılmıştır ve `matplotlib`’den nokta (.) ile erişilmektedir.

#### **Fonksiyon İçe Aktarma**

Kütüphane ve modüllerin yanı sıra, doğrudan belirli fonksiyonları da içe aktarabilirsiniz. Bu işlem için özel bir sözdizimi kullanılır. Örneğin:

```python
from sklearn.metrics import precision_score, recall_score
```

Burada içe aktarma işlemi **from** anahtar kelimesiyle başlar. `sklearn.metrics` ifadesi scikit-learn kütüphanesi ve içindeki `metrics` modülünü ifade eder. Sonra `import` anahtar kelimesi ve ardından istenilen fonksiyonlar yazılır: `precision_score` ve `recall_score`.

Aynı sözdizimi NumPy’nin `array()` fonksiyonu için de kullanılabilir. Ancak, NumPy'den genellikle tek tek fonksiyonlar yerine tüm kütüphane içe aktarılır. Yine de örnek olarak şöyle yazabiliriz:

```python
from numpy import array
```

Bu şekilde bir fonksiyon ismiyle içe aktarılırsa, fonksiyonu doğrudan şu şekilde kullanabilirsiniz:
...
Karşılaşabileceğiniz bir başka sözdizimi ise:

```python
from library.module import *
```

Bu, belirli bir kütüphane veya modülden her şeyi içe aktarır ve herhangi bir ön ek kullanmadan fonksiyonları doğrudan kullanmanıza olanak tanır. Örneğin:

```python
from numpy import *
```

Bu ifadeyi kullanırsanız, NumPy’nin tüm fonksiyonlarını `numpy.` veya `np.` yazmadan kullanabilirsiniz. **Ancak bu yöntem önerilmez**, çünkü hangi fonksiyonun nereden geldiğini takip etmeyi zorlaştırır. Yine de bu sözdizimine iş yerinde ya da örnek projelerde rastlayabileceğiniz için bilmek faydalı olacaktır.

## **Yaygın Olarak Kullanılan Yerleşik (Built-in) Modüller**

Python’un standart kütüphanesi, kurulumla birlikte gelen birçok yerleşik modül içerir. Bunlar ek bir kurulum gerektirmeden doğrudan içe aktarılabilir. Veri profesyonelleri için sık kullanılan bazı yerleşik modüller şunlardır:

### [datetime](https://docs.python.org/3/library/datetime.html#module-datetime)

* Tarih ve saat dönüşümleri ve hesaplamaları için kullanılır.
...
* Matematiksel işlemlere erişim sağlar.
...
print(math.sqrt(100))      # 100’ün karekökü
...
* Sahte rastgele sayılar üretmek için kullanılır (detaylar için dökümana bakınız).
...
print(random.choice([1, 2, 3]))  # listeden rastgele seçim
...
Kütüphaneler, paketler ve


modüller Python’un sunduğu sayısız işlevin kapılarını açar. Bunları nasıl kullanacağınızı öğrenmek, çalışma verimliliğinizi büyük ölçüde artırır. Kullanışlı kütüphaneler bulmak için [PyPI](https://pypi.org/) (Python Package Index) deposunu inceleyin. Kimya, ses düzenleme, doğal dil işleme, video oyunları ve daha fazlası için geliştirilmiş paketler vardır. Ne yapmaya çalışıyorsanız, büyük ihtimalle biri bunun için bir araç geliştirmiştir!

# Python’un Yeni Sürümleri ve Özellikleri

Python’un en büyük güçlü yönlerinden birinin dinamizmi olduğunu öğrendiniz: Geniş bir kullanıcı topluluğu tarafından geliştiriliyor ve destekleniyor. Bu da onun sürekli değişen ve evrilen bir dil olmasını sağlıyor. Yani, Python dili ve kütüphaneleri sürekli olarak büyüyor, gelişiyor ve kullanıcı geri bildirimlerine yanıt veriyor.

Python’un bu dinamik yapısı bir avantaj olsa da, bazı karmaşıklıklar da getirebilir. Bu durum, kodun uzun süre boyunca kullanılmasının hedeflendiği durumlarda ya da birden fazla kişinin birlikte çalıştığı projelerde bazı zorluklara yol açabilir. Belirli bir Python sürümünde geliştirilen bir betik ya da program, genellikle o ortama bağlı olur ve farklı bir sürümde çalıştırıldığında beklenmedik davranışlar sergileyebilir. Bunun nedeni, Python’un ve kütüphanelerinin zamanla güncellenmesidir. Örneğin, 2013 yılında Python 2.7.4 ile geliştirilmiş bir projeyi bugün Python 3.12.4 ile çalıştırmaya çalışırsanız, aynı şekilde çalışmayabilir. Benzer şekilde, ekip üyelerinin farklı kütüphane sürümleri kullanması, uyumluluk sorunlarına neden olabilir.

## **Kaldırılma (Deprecation)**

Kodlar zamanla kaçınılmaz olarak değişir, kullanımdan kaldırılır ve eskir. Bu sürece **kaldırılma (deprecation)** denir. Eğer bir Jupyter defterinde çalışıyorsanız, gelecekteki bir sürümde kaldırılması planlanan bir şeyi kullandığınızda genellikle bir uyarı alırsınız. Kodunuz hala çalışır, ancak gelecekteki sürümlerde çalışmayabilir ya da davranışı değişebilir. Bu uyarılara dikkat etmek ve kodunuzu yaklaşan değişikliğe uygun şekilde düzenlemek en iyisidir. Çünkü kodu yazarken bu tür değişiklikleri yapmak, ileride kod bozulduğunda ne yaptığınızı hatırlayıp düzeltmeye çalışmaktan çok daha kolaydır.

Ayrıca, küçük güncellemeler (örneğin, Python 3.11.8’den 3.11.9’a) büyük güncellemeler (örneğin, Python 2.X’ten 3.X’e) kadar sarsıcı olmaz. Bu nedenle büyük sürüm geçişlerinde kaldırılma (deprecation) mesajlarını daha sık görürsünüz.

Unutmayın, Python’un ve kütüphanelerinin eski sürümlerini hâlâ kullanabilirsiniz. Yeni bir sürüm çıktığında kodunuzu hemen güncellemeniz gerekmez. Genellikle eski sürümler, daha yenileri çıkmış olsa da uzun bir süre desteklenmeye devam eder. Bu da bireylerin ve kurumların kodlarını zaman içinde planlı şekilde güncelleyebilmesine olanak tanır. Siz belki projenizi kolayca güncelleyebilirsiniz, ancak büyük bir şirketin tüm kod tabanını güncellemesi ve hiçbir şeyin bozulmadığından emin olması çok daha zordur.

## **Ortamlar (Environments)**

Kod geliştirmek için kullandığınız araçlar, kütüphaneler, bağımlılıklar ve ayarların tümü **ortamınız** olarak adlandırılır. Ortam, kodu yazarken ve çalıştırırken kullanılan tüm yapılandırmalardır. Bir projede bir grup insanla birlikte çalışıyorsanız, herkesin aynı ortamı paylaşması önemlidir. Böylece kod, herkesin bilgisayarında aynı şekilde çalışır.

Eğer bu kursun Coursera’daki uygulamalı alıştırmalarını yapıyorsanız, ortamınız önceden yapılandırılmış olduğundan bu konuda endişelenmenize gerek yoktur. Ancak alıştırmaları Coursera dışındaki bir ortamda yapıyorsanız, kodunuzun çalışması Coursera'dakinden farklı olabilir.

Ortam yönetimi karmaşık ve teknik bir süreç olabilir. Bu kursta buna çok fazla değinilmeyecek, çünkü sizin için yapılandırılmış bir ortamda çalışıyorsunuz ve odak noktanız başka beceriler kazanmak olacak. Ancak kendi geliştirme ortamınızı oluşturmak ve yönetmek hakkında daha fazla bilgi edinmek isterseniz, [Anaconda Navigator](https://docs.anaconda.com/navigator/) iyi bir başlangıç noktasıdır.

**Anaconda Navigator**, ücretsiz ve açık kaynaklı, indirilebilir bir grafik arayüzdür. Ortamları kolayca yönetebilmenizi sağlar. Ayrıca Jupyter Notebook, PyCharm, VSCode ve veri profesyonelleri tarafından yaygın olarak kullanılan diğer araçlarla da entegredir.

## **Hangi Sürümleri Kullandığınızı Kontrol Edin**

Bu kurstaki kodların güncel kalmasını sağlamak için Python’un ve ilgili kütüphanelerin yeni kararlı sürümleri kullanılmaya çalışılır. Ancak her yeni sürüm çıktığında tüm kursları baştan sona güncellemek mümkün ya da gerekli değildir. Eğer Jupyter Notebook etkinliklerini kendi bilgisayarınızda yapıyorsanız, Coursera’daki örneklerle kıyaslandığında kodunuzun davranışında bazı farklılıklar görebilirsiniz. Bu farklar genellikle **sürüm farklılıklarından** kaynaklanır.

Bir Jupyter Notebook içinde kullandığınız Python sürümünü kontrol etmek için aşağıdaki kodu çalıştırabilirsiniz:
...
Kullandığınız bir kütüphanenin sürümünü kontrol etmek için, kütüphaneyi içe aktarıp `__version__` niteliğini kullanabilirsiniz. Örneğin:
...
Python, kullanıcılarına ve kullanım alanlarına göre gelişmeye devam eder. Bu da eski sürümlerle yazılmış kodların davranışlarının yeni sürümlerde değişebileceği anlamına gelir. Kullandığınız ortamı anlamak, yönetmek ve olası sorunları çözebilmek, yazılım geliştirmenin önemli bir parçasıdır.

# Referans Kılavuzu: Diziler

Daha önce öğrendiğiniz gibi, NumPy gelişmiş sayısal hesaplamalar yapabilen güçlü bir kütüphanedir. En önemli avantajlarından biri dizilerle (arrays) çalışma yeteneğidir çünkü bir vektöre uygulanan bir işlem, aynı işlemin bir listeye uygulanmasından çok daha hızlı gerçekleşir. Bu performans farkı, özellikle büyük veri hacimleriyle çalışırken çok daha belirgin hâle gelir. Bu okuma, NumPy dizileriyle çalışmak için bir referans rehberidir.

---

## **Dizi Oluşturma**

NumPy kullanmak için öncelikle onu içe aktarmanız gerekir. Yaygın kullanım, kütüphaneye `np` takma adı (alias) vermektir.

### [**np.array()**](https://numpy.org/doc/stable/reference/generated/numpy.array.html)

Bu yöntem çok boyutlu bir dizi (ndarray) oluşturur. NumPy dizilerinin kaç boyutlu olabileceğiyle ilgili bir sınır yoktur; ancak boyut sayısı arttıkça, diziyle çalışmak zorlaşabilir.
...
Bir boyutlu diziler, listelere benzer yapıdadır.
...
İki boyutlu diziler, tablo yapısına benzer.
...
Üç boyutlu diziler, birden fazla tabloyu temsil eder.
...
* Belirtilen şekle ve belirli bir değere sahip bir dizi oluşturur:
...
### Bu fonksiyonlar şunlar için faydalıdır:

* Belirli bir boyuta sahip diziler başlatmak
* Bellek ayırmak
* Matris işlemleri yapmak

---

## **Dizi Metotları**

NumPy dizileriyle birçok işlem yapmak için çeşitli metotlar mevcuttur. Bazı yaygın metotlar şunlardır:

### [flatten()](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.flatten.html)

Diziyi tek boyuta indirger:
...
Dizi elemanlarını değiştirmeden şeklini değiştirir:
...
`-1` değeri, boyutun otomatik hesaplanmasını sağlar:
...
* `mean()`: Ortalama değeri döner
* `min()`: En küçük değeri döner
* `std()`: Standart sapmayı döner
...
* `shape`: Dizi boyutlarını döner
* `dtype`: Veri tipini döner
* `size`: Eleman sayısını döner
* `T`: Transpoze (satırları sütun yapar)
...
Dizilerde belirli elemanlara ulaşmak için indeksleme ve dilimleme kullanılır:
...
Alt dizilere ulaşmak için dilimleme yapılabilir:
...
Diziler üzerinde aritmetik işlemler yapılabilir:
...
NumPy, 100'e yakın matematiksel fonksiyon sunar. [Tüm liste](https://numpy.org/doc/stable/reference/routines.math.html#mathematical-functions)

---

## **Değiştirilebilirlik (Mutability)**

NumPy dizileri değiştirilebilir, fakat boyutları değiştirilemez:
...
### NumPy Dizileri Bellekte Nasıl Saklanır?

NumPy dizileri, bellekte bitişik bir blokta saklanır. Bu yapı:

* **Hızlı erişim sağlar**
* **Dizi boyutunun değiştirilememesine neden olur**

![image](images/2005.png)

Yeni eleman eklemek için diziyi yeni bir bellek adresine kopyalamak gerekir.

# Pandas'ın Temelleri

Python’un, NumPy ve pandas gibi birçok açık kaynaklı kütüphane ve pakete sahip olduğunu öğrendiniz; bu da onu en kullanışlı programlama dillerinden biri haline getiriyor. Bu içerikte, pandas DataFrame’lerinin temellerini gözden geçirecek ve bunlarla nasıl çalışılacağını daha ayrıntılı şekilde öğreneceksiniz. Pandas’ın temellerini anlamak, yetkin bir veri profesyoneli olmak için çok önemlidir.

---

## **Temel Veri Yapıları**

Pandas kütüphanesinin iki ana veri yapısı vardır: Series (Seri) ve DataFrame.

* **Series (Seri):** Series, etiketli tek boyutlu bir dizidir ve herhangi bir veri türünü içerebilir. Bir e-tablo sütununa veya bir boyutlu NumPy dizisine benzer. Her öğenin, “indeks” adı verilen bir etiketi vardır. Bu indeksler, veriye erişmeyi ve veriler üzerinde işlem yapmayı kolaylaştırır.

* **DataFrame:** DataFrame, etiketli iki boyutlu bir veri yapısıdır; yani bir tablo gibidir. Her sütun ve satır aslında bir Series’tir.

---

## **Bir DataFrame Oluşturmak**

Pandas'ı kullanmak için önce içe aktarılmalıdır. NumPy gibi, pandas da genellikle `pd` takma adıyla içe aktarılır:

```python
import pandas as pd
```

Pandas içe aktarıldıktan sonra, bir DataFrame oluşturabilirsiniz. İşte Jupyter Notebook içinde DataFrame oluşturmanın birkaç yolu:
...
(Not: Bu kod çalışmaz çünkü gerçek bir dosya yoluna ihtiyaç duyar, sadece sözdizimini göstermek içindir.)

```python
df3 = pd.read_csv('/dosya_yolu/dosya_adi.csv')
```

---

## **Öznitelikler (Attributes) ve Metotlar (Methods)**

DataFrame sınıfı, veri analizi için çok sayıda yerleşik özellik (öznitelik ve metot) ile birlikte gelir.

* **Öznitelik (Attribute):** Bir nesneye ait bir özelliktir ve `.` (nokta) kullanılarak çağrılır.
* **Metot (Method):** Bir sınıfın içinde tanımlanmış fonksiyondur ve genellikle bir işlem gerçekleştirir.

### **Sık Kullanılan DataFrame Öznitelikleri**

| **Öznitelik** | **Açıklama**                                                 |
| ------------- | ------------------------------------------------------------ |
| `columns`     | DataFrame’in sütun etiketlerini döner                        |
| `dtypes`      | Veri türlerini döner                                         |
| `iloc`        | Tamsayı bazlı indekslemeyle satır ve sütunlara erişim sağlar |
| `loc`         | Etiket veya koşula göre satır ve sütunlara erişim sağlar     |
| `shape`       | DataFrame’in boyutlarını bir demet (tuple) olarak döner      |
| `values`      | DataFrame’in NumPy dizisi temsilini döner                    |

### **Sık Kullanılan DataFrame Metotları**

| **Metot**                | **Açıklama**                                                              |
| ------------------------ | ------------------------------------------------------------------------- |
| `apply()`                | Bir işlevi DataFrame boyunca uygular                                      |
| `copy()`                 | DataFrame’in kopyasını oluşturur                                          |
| `describe()`             | Sayısal sütunlar için temel istatistikleri verir (min, max, ortalama vs.) |
| `drop()`                 | Belirtilen satır ya da sütunları siler                                    |
| `groupby()`              | Veriyi gruplayıp işlev uygular                                            |
| `head(n=5)`              | İlk `n` satırı döner (varsayılan 5)                                       |
| `info()`                 | DataFrame’in genel özetini verir                                          |
| `isna()` veya `isnull()` | Null (boş) olan değerleri gösteren bir Boolean DataFrame döner            |
| `sort_values()`          | Belirli bir sütuna göre sıralama yapar                                    |
| `value_counts()`         | Eşsiz satırların sayısını verir                                           |
| `where()`                | Koşulun sağlanmadığı yerlerde değerleri değiştirir                        |

Bu sadece bazı temel öznitelik ve metotlardır. Daha fazlası için [pandas DataFrame belgelerine](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html) göz atabilirsiniz.

---

## **Seçim İfadeleri (Selection Statements)**

Veriniz DataFrame’e aktarıldıktan sonra, genellikle satır ve sütunları seçmek, düzenlemek ve analiz etmek istersiniz. Bu bölümde satır, sütun ve alt veri kümelerini nasıl seçeceğinizi öğreneceksiniz.
...
`loc[]`, satırları **isimle** seçmek için kullanılır.

```python
df.loc['row_1']
```

Liste olarak verirseniz, sonuç yine bir DataFrame olur:
...
Aralık belirtirken `loc[]`, **bitiş indeksini dahil eder**:
...
Aralık belirtme (bitiş dahil **değildir**):
...
```python
df['C']         # Tek sütun
df[['A', 'C']]  # Birden fazla sütun
```

### Nokta notasyonu:

```python
df.A  # Ancak tüm sütun adlarında geçerli olmayabilir
```
...
## **Satır ve Sütunları Birlikte Seçmek**
...
> **Uyarı:** `loc[]` ile karışık biçimde sayı ve etiket kullanmak hata üretir.

---

### Sayısal İndeksli DataFrame Örneği:
...
* Pandas DataFrame, tablo verileriyle çalışmak için ideal bir yapıdır.
* Satırlar ve sütunlar, Series (Seri) nesneleridir.
* Birçok yararlı metot ve öznitelikle birlikte gelir.
* `loc[]` ve `iloc[]` veri seçimi için önemli araçlardır.
* Uygulama yaptıkça bu notasyonlara alışacak ve pandas’ı daha etkili kullanacaksınız.

---

## **Daha Fazla Bilgi için Kaynaklar**

* [pandas DataFrame belgeleri](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.html)
* [pandas Series belgeleri](https://pandas.pydata.org/docs/reference/series.html)
* [pandas seçim belgeleri](https://pandas.pydata.org/docs/user_guide/10min.html#selection)

# Pandas'ta Boolean Masking

Artık pandas'ta satır ve sütunlara referans vererek veri seçmeyi öğrendiğinize göre, bir sonraki adım Boolean maskeler kullanmayı öğrenmektir. Veri profesyonelleri, pandas'ta koşullara göre veri seçmek için Boolean maskeler kullanır. Bu okumada Boolean maskeleri keşfedecek ve pandas'ın mantıksal (lojik) operatörlerini kullanarak çok koşullu seçim ifadeleri oluşturmayı öğreneceksiniz. Pandas’ın temellerini anlamak, bir veri uzmanı olarak işinizi daha kolay ve verimli hale getirecektir.

---

## **Boolean Maskeler**

Boolean (mantıksal) terimi, yalnızca doğru (**True**) ya da yanlış (**False**) iki değer alabilen değişkenleri tanımlar. Pandas’ta **Boolean masking** (mantıksal maskeleme), bir dataframe’in indeksine bir Boolean ızgara yerleştirilerek, yalnızca bu ızgaradaki **True** olan değerlere karşılık gelen satırların seçilmesi işlemidir. Buna **Boolean indexing** (mantıksal indeksleme) de denir.

Videodaki örneğe geri dönelim. Elinizde gezegenlerin, yarıçaplarının ve uydularının sayısının olduğu bir veri çerçevesi olduğunu varsayalım:

| **Gezegen** | **Yarıçap (km)** | **Uydu sayısı** |
| ----------- | ---------------- | --------------- |
| Merkür      | 2.440            | 0               |
| Venüs       | 6.052            | 0               |
| Dünya       | 6.371            | 1               |
...
Şimdi 20’den az uydusu olan gezegenleri tutmak ve geri kalanları filtrelemek istediğinizi varsayalım. Bu durumda bir Boolean maske, “moons” sütunundaki her değerin koşulu sağlayıp sağlamadığına göre True/False döndüren bir pandas **Series** nesnesi olacaktır:
...
Bu **Series**'in veri tipi `bool`'dur. Bu Boolean maske, dataframe'in indeksine yerleştirilir. Sonuç olarak, maskede **False** olan satırlar filtrelenir, **True** olanlar kalır:
...
### **Pandas'ta Boolean Maskeleri Kodlamak**

Bu işlemi pandas'ta şu şekilde gerçekleştirebilirsiniz:

**Önce bir DataFrame oluşturun:**
...
**Bir mantıksal ifade yazın:**

```python
print(df['moons'] < 20)
```

Bu kod size, her satırın koşulu sağlayıp sağlamadığına göre **True/False** değerlerinden oluşan bir Series döndürecektir. Bu Boolean maskeyi, doğrudan dataframe’inizin seçim parantezine koyabilirsiniz:

```python
print(df[df['moons'] < 20])
```

**Ya da maskeyi bir değişkene atayabilirsiniz:**
...
**Bu işlem dataframe'inizi kalıcı olarak değiştirmez.** Yalnızca filtrelenmiş bir görünüm sağlar. Ancak isterseniz sonucu yeni bir değişkene atayabilirsiniz:
...
**Sadece gezegen isimlerini seçmek isterseniz `loc[]` kullanabilirsiniz:**
...
Birden fazla koşul içeren ifadelerde pandas, aşağıdaki **mantıksal operatörleri** kullanır:
...
**Önemli:** Her bir koşul **parantez içine alınmalıdır.** Aksi takdirde hata alabilirsiniz veya yanlış sonuçlar dönebilir.

Örneğin, 10'dan az **veya** 50’den fazla uydusu olan gezegenleri seçmek için:
...
**Örnek - Daha karmaşık bir ifade:**

20’den fazla uydusu olan **fakat** uydusu **tam olarak 80 olmayan** ve yarıçapı **50.000 km’den küçük olmayan** gezegenleri seçmek:
...
* **Boolean mask** bir dataframe'e filtre uygulamak için kullanılan bir yöntemdir.
* Maske, yalnızca True olan satırları tutar, False olanları dışlar.
* Pandas’ta mantıksal karşılaştırmalar yapmak için kullanılan operatörler:

  * `&` (ve)
  * `|` (veya)
  * `~` (değil)
* Çoklu koşullarda her bir koşul **parantez içine** alınmalıdır.
* Uygulama yaparak, pandas’ta karmaşık seçim ifadeleri yazmak kolay ve verimli hale gelir.

---

## **Daha Fazla Bilgi İçin Kaynaklar**

* [pandas Boolean indexing dökümantasyonu](https://pandas.pydata.org/docs/user_guide/indexing.html#boolean-indexing)

# Gruplama ve birleştirme hakkında daha fazla bilgi

Pandas'ın tablo şeklindeki verileri incelemeyi ve değiştirmeyi kolaylaştıran bir Python kütüphanesi olduğunu keşfettiniz. Ayrıca groupby() ve agg(), veri uzmanlarının verileri gruplamak, toplamak, özetlemek ve daha iyi anlamak için kullandıkları temel DataFrame yöntemleridir. Bu okumada, bu fonksiyonların nasıl çalıştığını ve ne zaman ve nasıl uygulanacağını gözden geçireceksiniz.

## groupby()

[groupby()](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.groupby.html) işlevi DataFrame sınıfına ait bir yöntemdir. Verileri belirtilen kriterlere göre gruplara ayırarak, her gruba bağımsız olarak bir fonksiyon uygulayarak ve ardından sonuçları bir veri yapısında birleştirerek çalışır. Bir veri çerçevesine uygulandığında, fonksiyon bir groupby nesnesi döndürür. Bu groupby nesnesi, aşağıdakiler de dahil olmak üzere farklı veri manipülasyon işlemleri için temel görevi görür:

-   Toplama: Her grup için özet istatistiklerin hesaplanması
    
-   Dönüşüm: Her gruba fonksiyonların uygulanması ve değiştirilmiş verilerin döndürülmesi
    
-   Filtreleme: Belirli koşullara göre belirli grupların seçilmesi
    
-   Yineleme: Gruplar veya değerler üzerinde yineleme
    

Aşağıda, farklı giyim eşyalarından oluşan bir veri çerçevesi üzerinde groupby() işlevini kullanan bazı örnekler verilmiştir:
...
Veri çerçevesinin türe göre gruplandırılması bir DataFrameGroupBy nesnesi ile sonuçlanır:
...
# <pandas.core.groupby.DataFrameGroupBy object at 0x7f4b5e923160>
# <class 'pandas.core.groupby.DataFrameGroupBy'>
```

Ancak groupby nesnesine bir toplama işlevi uygulanabilir:
...
Önceki örnekte, groupby() tüm öğeleri türlerine göre gruplar halinde birleştirmiş ve veri çerçevesindeki her sayısal sütun için her grubun ortalamasını içeren bir DataFrame nesnesi döndürmüştür. Not: Pandas'ın gelecekteki sürümlerinde, groupby nesnesine mean gibi belirli toplama işlevlerini uygularken bir numeric_only parametresi belirtmek gerekecektir. numeric_only, her sütunun veri türünü ifade eder. Pandas'ın önceki sürümlerinde (bu platformdaki sürüm gibi) numeric_only=True belirtmek gerekli değildir, ancak gelecek sürümlerde bu yapılmalıdır. Aksi takdirde, yakalanacak belirli sütunların belirtilmesi gerekecektir)

Ayrıca, birden fazla sütuna dayalı olarak gruplar oluşturulabilir
...
Önceki örnekte groupby() doğrudan clothes veri çerçevesi üzerinde çağrılmıştır. Veriler önce türe, sonra da renge göre gruplandırılmıştır. Bunun sonucunda dört grup ortaya çıkmıştır - tip ve renk için mevcut farklı değer kombinasyonlarının sayısı. Ardından, her grubu minimum değerine göre filtrelemek için sonuca min() işlevi uygulandı.

Her grupta bulunan gözlem sayısını basitçe döndürmek için size() yöntemini kullanın. Bu, ilgili bilgileri içeren bir Seri nesnesi ile sonuçlanacaktır:
...
Önceki örneklerde groupby nesnelerine uygulanan mean(), min() ve size () toplama işlevleri gösterilmiştir. Kullanılabilir birçok yerleşik toplama işlevi vardır. En yaygın kullanılanlardan bazıları şunlardır:
...
-   median(): Her gruptaki değerlerin ortancası
...
[agg()](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.agg.html) işlevi, bir veri çerçevesine aynı anda birden fazla işlev uygulamak istediğinizde kullanışlıdır. agg( ), DataFrame sınıfına ait bir yöntemdir. "Toplam" anlamına geliyor. En önemli parametreleri şunlardır:  
...
Aşağıda agg() fonksiyonunun nasıl kullanılabileceğine dair bazı örnekler verilmiştir. Bu fonksiyonun kendi başına ( groupby() olmadan) nasıl kullanılabileceğini gösterdiklerine dikkat edin. Platform sınırlamaları nedeniyle aşağıdaki kod bloklarından bazılarının çalıştırılamayacağını da unutmayın. Bu durumlarda, çıktı bir görüntü olarak sağlanır. Hatırlatma amacıyla orijinal giysi veri çerçevesini tekrar burada bulabilirsiniz:
...
Aşağıdaki örnek, sum() ve mean() fonksiyonlarını clothes veri çerçevesinin price ve mass_g sütunlarına uygular.
...
-   İki sütun, agg() yöntemi uygulanmadan önce veri çerçevesinden alt kümeye alınır. Önce ilgili sütunların alt kümesini oluşturmazsanız, agg( ) _tüm_ sütunlara sum () ve mean() uygulamaya çalışacaktır, ancak bazı sütunlar dizeler içerdiğinden bu işe yaramayacaktır. (Teknik olarak sum() işe yarayacaktır, ancak tüm dizeleri tek bir uzun dizede birleştireceği için işe yaramaz bir şey döndürecektir).
    
-   sum() ve mean() fonksiyonları, parantezleri olmadan bir liste içinde dizeler olarak girilir. Bu, herhangi bir yerleşik toplama işlevi için çalışacaktır.
    

Bir sonraki örnekte, farklı sütunlara farklı fonksiyonlar uygulanmaktadır.
...
-   Sütunlar, agg() işlevi uygulanmadan önce veri çerçevesinden alt kümeye alınmaz. Bu gereksizdir çünkü sütunlar agg() işlevinin içinde belirtilir.
    
-   agg() fonksiyonunun argümanı, anahtarları sütunlar ve değerleri de bu sütunlara uygulanacak fonksiyonlar olan bir sözlüktür. Bir sütuna birden fazla fonksiyon uygulanırsa, bunlar bir liste olarak girilir. Yine, her yerleşik işlev parantezsiz bir dize olarak girilir.
    
-   Sonuçta ortaya çıkan veri çerçevesi, belirli bir işlevin kullanılmak üzere belirlenmediği NaN değerlerini içerir.
    

Aşağıdaki örnek, sum() ve mean() fonksiyonlarını eksen 1'e uygular. Başka bir deyişle, fonksiyonlar her sütuna uygulanmak yerine her satıra uygulanır.
...
groupby() ve agg() fonksiyonları genellikle birlikte kullanılır. Bu gibi durumlarda, önce groupby() işlevini bir veri çerçevesine uygulayın, ardından agg() işlevini groupby'nin sonucuna uygulayın. Referans için, giysi veri çerçevesini bir kez daha burada bulabilirsiniz.
...
Aşağıdaki örnekte, giysilerdeki öğeler renklerine göre gruplandırılır, ardından bu grupların her birine price_usd ve mass_g sütunlarında mean() ve max() işlevleri uygulanır.
...
İşlevler bir groupby nesnesine uygulandığında, ortaya çıkan veri çerçevesinin katmanlı indekslere sahip olduğunu fark etmiş olabilirsiniz. Bu bir **MultiIndex** örneğidir. MultiIndex, hiyerarşik bir veri çerçevesi indeksleme sistemidir. Seriler ve veri çerçeveleri gibi daha düşük boyutlu veri yapılarında istediğiniz sayıda boyuta sahip verileri saklamanıza ve işlemenize olanak tanır. Bu, karmaşık veri manipülasyonunu kolaylaştırır. 

Bu eğitim hiyerarşik indeksleme hakkında derin bilgi sahibi olmayı gerektirmez, ancak bu konuda bilgi sahibi olmak faydalı olacaktır. Aşağıdaki örneği ele alalım:
...
Renk ve türün çıktıda sütun adlarından daha aşağıda konumlandırıldığına dikkat edin. Bu, renk ve türün artık sütun değil, adlandırılmış satır indisleri olduğunu gösterir. Benzer şekilde, price_usd ve mass_g 'nin sütun adları çıktısında mean ve min 'in üzerinde konumlandırıldığına dikkat edin, bu da hiyerarşik bir sütun dizinine işaret eder. 

Satır indeksini incelerseniz, satır indeksleri hakkında bilgi içeren bir MultiIndex nesnesi elde edersiniz:
...
Sütun dizini, sütun dizinleri hakkında bilgi içeren bir MultiIndex nesnesini gösterir:
...
MultiIndex ile bir veri çerçevesi üzerinde seçim yapmak için loc[] selection kullanın ve indisleri parantez içine alın. İki seviyeli satır indeksi ve iki seviyeli sütun indeksi olan bir veri çerçevesi olan grouped ile ilgili bazı örnekler aşağıda verilmiştir. Referans için, işte gruplandırılmış veri çerçevesi:
...
Birinci seviye (üst) bir sütun seçmek için:
...
İkinci seviye (alt) bir sütun seçmek için:
...
Alt düzey (en sağ) bir satır seçmek için:
...
Ve hatta tek tek değerleri bile seçebilirsiniz:
...
Bir groupby sonucundan MultiIndex satırını kaldırmak istiyorsanız, groupby() deyiminize parametre olarak as_index=False değerini ekleyin:
...
Renk ve türün artık satır indisleri değil, adlandırılmış sütunlar olduğuna dikkat edin. Satır indisleri sıfırdan başlayan standart numaralandırmadır. 

Yine, bu derste hiyerarşik olarak indekslenmiş veriler üzerinde karmaşık manipülasyonlar yapmanız beklenmemektedir, ancak özellikle groupby() manipülasyonları varsayılan olarak bir MultiIndex veri çerçevesi ile sonuçlandığından, MultIndex'in nasıl çalıştığına dair temel bir anlayışa sahip olmanız yararlı olacaktır. 

## Önemli çıkarımlar

groupby(), verilerin verimli bir şekilde birleştirilmesini ve analiz edilmesini sağladığı için bir veri uzmanı olarak çalışmalarınızda önemli bir işlev olacaktır. Benzer şekilde agg(), bir veri çerçevesinin belirli bir ekseninde dinamik olarak birden fazla işlev uygulamanıza yardımcı olur. Bu araçlar tek başlarına ya da birlikte kullanıldıklarında veri uzmanlarının verilere derinlemesine erişimini sağlar ve başarılı projelerin ortaya çıkmasına yardımcı olur.
