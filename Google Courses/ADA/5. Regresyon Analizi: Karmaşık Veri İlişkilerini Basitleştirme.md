...
2. **Gerçek Pozitifler (True Positives):**
...
Lojistik regresyon sınıflandırıcısını değerlendirmede önemli metriklerden biri **kesinlik**tir. Kesinlik, "Doğru" olarak tahmin edilen veri noktalarının gerçekten ne kadarının doğru olduğunu ölçer. Örneğin, bir e-posta spam tespiti sınıflandırıcısı geliştirdiğinizi ve bu sınıflandırıcı ile e-postalar için tahminler yaptığınızı düşünün. Tahminler "Doğru" (spam olarak tahmin edilen) ve "Yanlış" (spam olmayan olarak tahmin edilen) değerlerden oluşur. Kesinlik, spam olarak tahmin edilen e-postaların kaçının gerçekten spam olduğunu gösterir.
...
Python’da kesinlik hesaplamak için sklearn kütüphanesinden metrics modülündeki `precision_score()` fonksiyonunu kullanabilirsiniz:
...
Eğer `precision_score()` fonksiyonu 0.91 dönerse, bu spam olarak tahmin edilen e-postaların %91’inin gerçekten spam olduğu anlamına gelir.

---

## Geri Çağırma (Recall)

Bir diğer önemli metrik **geri çağırma**dır. Geri çağırma, gerçekten doğru olan veri noktalarının ne kadarının doğru olarak tahmin edildiğini ölçer. Örneğin, dolandırıcılık tespiti için bir lojistik regresyon sınıflandırıcısı geliştirdiğinizi düşünün. Tahminlerde "Doğru", dolandırıcılık olarak tahmin edilen işlem; "Yanlış" ise dolandırıcılık olmayan işlem anlamına gelir. Geri çağırma, dolandırıcılık işlemlerinin ne kadarının doğru tespit edildiğini gösterir.

Geri çağırma formülü:

$$
\text{Geri Çağırma} = \frac{\text{Gerçek Pozitifler}}{\text{Gerçek Pozitifler} + \text{Yanlış Negatifler}}
$$

Python’da geri çağırma hesaplamak için:

```python
metrics.recall_score(y_test, y_pred)
```

Eğer `recall_score()` 0.87 dönerse, bu dolandırıcılık işlemlerinin %87’sinin doğru tespit edildiği anlamına gelir.

---

## Doğruluk (Accuracy)

Lojistik regresyonun değerlendirilmesinde önemli bir diğer metrik de **doğruluk**tur. Doğruluk, doğru sınıflandırılan veri noktalarının toplam veri noktalarına oranıdır. Örneğin, kredi onayı tahmini için bir sınıflandırıcı geliştirdiğinizi düşünün. Tahminlerde "Doğru", kredinin onaylanacağı tahmini; "Yanlış" ise onaylanmayacağı tahminidir. Doğruluk, doğru tahmin edilen kredi sayısının oranını verir.

Doğruluk formülü:

$$
\text{Doğruluk} = \frac{\text{Gerçek Pozitifler} + \text{Gerçek Negatifler}}{\text{Toplam Tahmin Sayısı}}
$$

Python’da doğruluk hesaplamak için:

```python
metrics.accuracy_score(y_test, y_pred)
```

Eğer `accuracy_score()` 0.90 dönerse, bu kredilerin %90’ının doğru tahmin edildiği anlamına gelir.

---

## ROC Eğrileri

**ROC eğrisi (Receiver Operating Characteristic curve)**, lojistik regresyon sınıflandırıcısının performansını görselleştirmeye yarar. Farklı sınıflandırma eşiklerinde sınıflandırıcının nasıl performans gösterdiğini anlamak için ROC eğrisi çizilir. İkili sınıflandırmada eşik, pozitif ve negatif sınıfları ayırmak için kullanılan sınırdır.

ROC eğrisi iki temel kavramı çizer:

1. **Gerçek Pozitif Oranı (True Positive Rate):** Geri çağırma ile aynıdır.

$$
\text{Gerçek Pozitif Oranı} = \frac{\text{Gerçek Pozitifler}}{\text{Gerçek Pozitifler} + \text{Yanlış Negatifler}}
$$

2. **Yanlış Pozitif Oranı (False Positive Rate):** Yanlış pozitiflerin, aslında negatif olması gereken gözlemler içindeki oranı.

$$
\text{Yanlış Pozitif Oranı} = \frac{\text{Yanlış Pozitifler}}{\text{Yanlış Pozitifler} + \text{Gerçek Negatifler}}
$$

ROC eğrisindeki her noktanın x ve y koordinatları sırasıyla yanlış pozitif oranı ve gerçek pozitif oranıdır.

![image](images/5026.png)

Python’da ROC eğrisi çizmek için:
...
![image](images/5027.png)

---

## AUC

**AUC (Area Under the Curve)**, ROC eğrisinin altında kalan alanı ifade eder. AUC, tüm sınıflandırma eşiklerinde model performansının genel bir ölçüsüdür. Değeri 0.0 ile 1.0 arasında değişir. 0.0 modeli tamamen yanlış sınıflandırıyor, 1.0 modeli ise tamamen doğru sınıflandırıyor demektir. 0.5’in altında AUC değeri, modelin rastgele sınıflandırıcıdan daha kötü performans gösterdiğini gösterir.

![image](images/5028.png)

Python’da AUC hesaplamak için:

```python
metrics.roc_auc_score(y_test, y_pred)
```

Örneğin, spam tespiti sınıflandırıcısı için `roc_auc_score()` 0.99 dönerse, bu modelin tüm eşiklerde %99 doğru tahmin yaptığı anlamına gelir.

---

## Özet

* Kesinlik, geri çağırma ve doğruluk, lojistik regresyon sınıflandırıcılarının performansını değerlendirmek için yaygın metriklerdir.
* Python’da sklearn kütüphanesinin metrics modülündeki fonksiyonlarla bu metrikler kolayca hesaplanabilir.
* ROC eğrisi çizmek, sınıflandırıcının farklı eşiklerdeki performansını görselleştirir.
* AUC hesaplamak, model performansını tek bir değerle özetler.

# Lojistik Regresyon Modellerini Yorumlama

Bir lojistik regresyon modelini yorumlamak, katsayıları incelemeyi ve metrikler hesaplamayı içerir. Lojistik regresyon modelinizi eğitim verisine uyguladıktan sonra, Python koduyla modelden katsayı tahminlerine erişebilirsiniz. Ardından, bu değerleri modelin nasıl tahmin yaptığını anlamak için kullanabilirsiniz. Bu okuma, bir lojistik regresyon modelinden katsayıların nasıl yorumlanacağını ve model değerlendirme için metrik seçerken nelere dikkat edilmesi gerektiğini gösterecektir.

## Modelden Katsayılar

Bir lojistik regresyon modelinin nasıl çalıştığını anlamak için, değişkenler arasındaki ilişkiyi tanımlayan denklemi incelemek önemlidir. Bu denklem aynı zamanda logit fonksiyonu olarak adlandırılır.

### **Logit Fonksiyonu**

Logit fonksiyonu bağımsız değişkenler cinsinden yazıldığında, şunu ifade eder: Her bağımsız değişken XXXX ile bağımlı değişken YYYY'nin 1 olma olasılığının logiti arasında doğrusal bir ilişki vardır. Bu olasılığın logiti, o olasılığın oranlarının (odds) logaritmasıdır.

İkili lojistik regresyonda logit fonksiyonunun denklemi aşağıdaki gibidir. Bu, YYYY'nin 1 olma olasılığını içerir çünkü 1, ikili sınıflandırmada (YYYY değerleri 1 ve 0 olabilir) tipik ilgi odağıdır.

$$
\text{logit}(p) = \log\left(\frac{p}{1-p}\right) = \beta_0 + \beta_1 X_1 \quad \text{burada } p = P(Y=1)
$$

### **Katsayıları Yorumlama**

Örneğin, e-postaların spam veya spam olmayan olarak sınıflandırılması için ikili lojistik regresyon modeli kurduğunuzu varsayalım. Bağımlı değişken YYYY, bir e-postanın spam (1) veya spam olmayan (0) olmasıdır. Bağımsız değişken X1, mesaj uzunluğudur. clf, eğitim verisine uydurduğunuz sınıflandırıcıdır.

Model tarafından tahmin edilen $\beta_1$ katsayısına erişmek için aşağıdaki kodu kullanabilirsiniz:

```python
clf.coef_
```

Örneğin tahmin edilen $\beta_1 = 0.186$ ise, bu mesaj uzunluğundaki bir birim artışın, $p$ olasılığının log-oranında 0.186 artışla ilişkili olduğu anlamına gelir. Y'nin oranlarındaki değişimi yüzde olarak yorumlamak için $\beta_1$ üssünü alabilirsiniz:

$$
e^{\beta_1} = e^{0.186} \approx 1.204
$$

Yani, mesaj uzunluğundaki her bir birim artış için, e-postanın spam olma olasılığının oranı 1.204 kat, yani %20.4 artar.

## Metrik Seçerken Dikkat Edilmesi Gerekenler

Lojistik regresyon modelinden katsayıları inceledikten sonra, modelin metriklerle değerlendirilmesi önemli bir adımdır. En yaygın kullanılan metrikler doğruluk (accuracy), kesinlik (precision) ve geri çağırma (recall) metrikleridir. Aşağıdaki bölümler, bu metrikler arasında seçim yaparken akılda tutulması gerekenleri açıklar.

### **Kesinlik Ne Zaman Kullanılır?**

Kesinlik, yanlış pozitiflerin (false positive) maliyetinin çok yüksek olduğu durumlarda özellikle yararlıdır ve yanlış negatiflerden (false negative) daha ağır basar. Örneğin, e-posta spam tespitinde, yanlış pozitif (spam olmayan bir e-postanın spam olarak tahmin edilmesi) yanlış negatiften daha maliyetlidir. Çünkü yanlış sınıflandırılan spam olmayan bir e-posta önemli bilgileri içerebilir (örneğin, bir tedarikçiden müşteriye proje durumu güncellemeleri veya bir öğretmenden öğrencilere ödev teslim tarihi bildirimleri).

### **Geri Çağırma Ne Zaman Kullanılır?**

Geri çağırma, yanlış negatiflerin maliyetinin çok yüksek olduğu ve yanlış pozitiflerden daha ağır basması gereken durumlarda faydalıdır. Örneğin, kredi kartı işlemlerinde dolandırıcılık tespitinde, yanlış negatif (dolandırıcılık içeren işlemin dolandırıcılık olmayan olarak tahmin edilmesi) yanlış pozitiften daha maliyetlidir. Yanlış sınıflandırılan bir dolandırıcılık müşterinin para kaybetmesine neden olabilir ve fark edilmeden kalabilir.

### **Doğruluk Ne Zaman Kullanılır?**

Doğruluk metriği, elinizdeki verinin ne kadarının doğru sınıflandırıldığını bilmek istediğinizde faydalıdır. Ayrıca, veri dengeliyse, yani pozitif ve negatif örneklerin sayısı yaklaşık eşitse doğruluk uygun bir metriktir. Ancak veri dengesizse, doğruluk yanıltıcı olabilir. Örneğin, verinin %95'i pozitif örnek, %5'i negatif örnek olsun. Bu veride bir model %95 doğrulukla sadece pozitif sınıfı tahmin ediyor olabilir ama bu modelin etkin olduğunu göstermez. Dengeli olmayan verilerde, veriyi dengelemek veya doğruluk dışındaki başka metrikleri kullanmak daha iyidir.

## Önemli Noktalar

* Bir modelden beta katsayılarını inceleyerek modelin bağımlı değişkeni nasıl tahmin ettiğini anlayabilirsiniz.
* Lojistik regresyon sınıflandırıcılarını değerlendirirken kullanılacak metriklerin seçimi, verinin bağlamı, tahminlerin nasıl kullanılacağı ve yanlış pozitiflerin ve yanlış negatiflerin etkileri göz önünde bulundurulmalıdır.

## Daha Fazla Bilgi İçin Kaynaklar

* [LogisticRegression (sklearn dokümantasyonu)](https://scikit-learn.org/stable/modules/generated/sklearn.linear_model.LogisticRegression.html): sklearn kullanarak lojistik regresyon modellerinin uygulanması ve modelden kesme ve katsayılara erişim hakkında dokümantasyon.

# Farklı Regresyon Türleri ile Tahmin Yapmak

Öğrendiğiniz gibi, veri profesyoneli olarak karşılaşacağınız temel regresyon teknikleri arasında doğrusal regresyon, hipotez testi ve lojistik regresyon yer alır. Veri ile tahmin yapmak istediğinizde, bu farklı yaklaşımları dikkate almak ve hangi yaklaşımın amacınıza en iyi şekilde hizmet edeceğini düşünmek önemlidir. Bu okumada, yanıtlamak istediğiniz soru, sonuç değişkeni ve bu değişkenin nasıl ölçüldüğüne göre bir projede en uygun regresyon tekniğini nasıl seçebileceğinizi öğreneceksiniz.

## Regresyon Tekniği Nasıl Seçilir?

Regresyon tekniği seçerken, üzerinde çalıştığınız veri ve yanıtlamak istediğiniz soruyu göz önünde bulundurmak önemlidir.

### **Dikkat Edilmesi Gerekenler**

1. Yanıtlamak istediğiniz soru nedir? Başka bir deyişle, neyi tahmin etmek istiyorsunuz?
2. Verinizde hangi değişken sonuç değişkeni olabilir?
3. Sonuç değişkeni nasıl ölçülüyor? Sonuç değişkeni sürekli ise, doğrusal regresyon veya hipotez testi daha uygun olabilir. Ancak sonuç değişkeni ikili (binary) ise, lojistik regresyon daha faydalı olacaktır.

## Regresyon için Örnek Bağlamlar

Aşağıdaki örnekler, tahmin soruları, sonuç değişkeni ve ölçüm hakkında verilen bilgilerin regresyon tekniği seçiminde nasıl yol gösterdiğini göstermektedir.

### **Örnek Bağlam: Kullanıcı Etkileşimi**

Bir veri profesyoneli olarak, bir mobil uygulamanın kullanıcı etkileşimini tahmin etmekle ilgilendiğinizi varsayalım.

Öncelikle, yanıtlamak istediğiniz soru nedir?

Bir soru şöyle olabilir: “Her bir uygulama içi özellik kullanıcı etkileşimini ne kadar etkiler?” Bu özellikler arasında müşteri destek canlı sohbeti, haftalık güncellenen SSS bölümü ve kullanıcıların birbirleriyle bağlantı kurabileceği bir topluluk alanı olabilir. Sonra, verinizde sonuç değişkeni olarak hangi değişken kullanılabilir diye sorabilirsiniz. Eğer kullanıcıların oturum sürelerine ilişkin veriye (yani kullanıcıların uygulamayı her açışlarında ne kadar zaman geçirdiklerine) erişiminiz varsa, sonuç değişkeni oturum süresi olabilir. Bir sonraki soru: sonuç değişkeni nasıl ölçülüyor? Oturum süresi dakika cinsinden ölçülebilir ve sürekli bir değişkendir. Sonuç değişkeni sürekli olduğundan ve her özelliğin sonuç üzerindeki etkisini öğrenmek istediğinizden, doğrusal regresyonla devam edebilir ve ilgili model varsayımlarını kontrol edebilirsiniz. Eğer tek bir özellik ilgi alanınızdaysa basit doğrusal regresyon modeli kurarsınız; birden fazla özellik varsa çoklu doğrusal regresyon modeli kurarsınız.

Başka bir ilgi alanı sorusu olabilir: “Dinamik bir açılış sayfası ile statik bir açılış sayfası, kullanıcı etkileşimini etkiler mi?” Bu örnekte de sonuç değişkeni dakika cinsinden oturum süresi olabilir. Sonuç değişkeni sürekli olduğundan ve hedef soru iki tür açılış sayfası arasında etkileşim farkı olup olmadığını sorduğundan, hipotez testi ile ilerleyebilirsiniz. Hipotezleri şu şekilde kurabilirsiniz:

* Null hipotezi (H₀): Kullanıcılar, açılış sayfası dinamik mi yoksa statik mi olduğuna bakılmaksızın uygulamada yaklaşık aynı süreyi geçirirler.
* Alternatif hipotez (H₁): Kullanıcılar, açılış sayfası dinamik veya statik olduğunda yaklaşık aynı süreyi geçirmezler.

Bir başka soru olabilir: “Bir kullanıcı uygulama içindeki yeni ürün serisiyle etkileşimde bulunur mu?” Sonuç değişkeni olarak verinizde bir kullanıcının yeni ürün serisini görüntülemek için tıklayıp tıklamadığı olabilir. Sonuç değişkeni nasıl ölçülüyor? Kullanıcının tıklayıp tıklamaması ikili bir değişken olarak 1 (tıkladı) veya 0 (tıkladı değil) şeklinde temsil edilir. Sonuç değişkeni ikili olduğundan, binom lojistik regresyonla devam edebilirsiniz.

### **Örnek Bağlam: Hasta Tepkisi**

Şimdi, tıbbi tedavilere hasta tepkilerini tahmin etmekle görevlendirildiğinizi düşünün.

İlk olarak, yanıtlamak istediğiniz soru nedir?

Bir olası soru: “Her bir faktör hastanın tedaviye verdiği tepkiyi ne kadar etkiler?” Tedavinin amacı beyaz kan hücresi (WBC) sayısını artırmaksa ve buna ilişkin veriniz varsa, WBC sayısı sonuç değişkeni olabilir. Sonuç değişkeni sürekli olduğundan, bu görev için doğrusal regresyon kullanabilirsiniz.

Başka bir soru olabilir: “Tedavi A, B veya C hastanın WBC sayısı üzerinde hangisi daha etkili olur?” Bu durumda da sonuç değişkeni WBC sayısıdır ve sürekli bir değişkendir. Hedef soru farklı tedavileri karşılaştırmak olduğundan, hipotez testi en uygun yaklaşımdır. Hipotezleri şu şekilde kurabilirsiniz:

* Null hipotezi (H₀): Hastaların WBC sayıları her tedavi için yaklaşık olarak aynıdır.
* Alternatif hipotez (H₁): Hastaların WBC sayıları her tedavi için yaklaşık olarak aynı değildir.

Farklı bir soru: “Tedavi A ile hastanın WBC sayısı ideal aralığa ulaşacak mı?” Eğer bu veriye erişiminiz varsa, sonuç değişkeni hastanın WBC sayısının ideal aralıkta olup olmaması olur; bu ikili bir değişkendir: 1 ideal aralıkta, 0 değil. Bu durumda lojistik regresyon modeli ile tahmin yapabilirsiniz.

## Önemli Noktalar

* Tahmin yapmak için regresyon tekniği seçerken yanıtlamak istediğiniz soru ve erişiminiz olan veriyi dikkate alın.
* İlgi duyduğunuz sonuç değişkenini ve nasıl ölçüldüğünü belirlemek, hangi regresyon tekniğinin görev için en uygun olduğunu anlamanıza yardımcı olur.

Aşağıdaki akış şeması, bu okumada tartışıldığı gibi, sonuç değişkeninden başlayarak regresyon tekniği seçmek için genel bir yaklaşımı gösterir. Hipotez testinin regresyon analizine bağlı olduğunu da unutmayın. Örneğin, doğrusal regresyonda iki değişken arasında korelasyon olup olmadığını test etmek (yani katsayıların istatistiksel olarak anlamlı olup olmadığını belirlemek) hipotez testi gerektirir.

![image](images/5029.png)

## Daha Fazla Bilgi İçin Kaynaklar

* Farklı regresyon modelleri hakkında daha fazla bilgi edinmek isterseniz, doğrusal regresyon, lojistik regresyon ve diğerleri hakkında bilgi veren [bu makaleyi](https://www.analyticsvidhya.com/blog/2022/01/different-types-of-regression-models/) inceleyebilirsiniz.
* Hipotez testi hakkında daha fazla bilgi için, veri bilimi bağlamında hipotez testi yaklaşımlarını ana hatlarıyla açıklayan [bu makaleye](https://towardsdatascience.com/hypothesis-testing-for-data-scientists-everything-you-need-to-know-8c36ddde4cd2) göz atabilirsiniz.
