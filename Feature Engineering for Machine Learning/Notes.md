...
Eğer istatistikler, daha fazla ve daha fazla tarihsel veri ile sürekli olarak güncellenirse, ham sayılar sınırsız bir şekilde büyüyecektir. Bu, model için bir problem olabilir. Eğitimli bir model, gözlemlenen ölçeğe kadar giriş verilerini "bilir". Eğitimli bir karar ağacı şöyle diyebilir: "x, 3'ten büyükse, 1 tahmin et." Eğitimli bir doğrusal model ise şöyle diyebilir: "x'i 0.7 ile çarp ve sonucu küresel ortalamadan büyük olup olmadığına bak." Bu kararlar, x'in 0 ile 5 arasında olduğu durumlar için doğru olabilir. Peki ya bunun ötesinde? Kimse bilmiyor.

Giriş sayıları arttıkça, modelin mevcut ölçeğe uyum sağlamak için yeniden eğitilmesi gerekecektir. Eğer sayılar yavaşça birikiyorsa, etkin ölçek çok hızlı değişmez ve model çok sık yeniden eğitilmeye ihtiyaç duymaz. Ancak sayılar çok hızlı artarsa, sık yeniden eğitim bir rahatsızlık haline gelir.

Bu nedenle, genellikle bilinen bir aralıkta sınırlandırılmış **normalleştirilmiş sayılar** kullanmak daha iyi bir çözümdür. Örneğin, tahmin edilen tıklama oranı [0, 1] arasında sınırlıdır. Diğer bir yöntem ise log dönüşümü kullanmaktır, bu yöntem sıkı bir sınır getirir, ancak sayılar çok büyük olduğunda artış hızı çok yavaş olur. Her iki yöntem de giriş dağılımlarındaki değişimlere karşı koruma sağlamaz (örneğin, geçen yılki Barbie bebekleri artık modası geçmiş ve insanlar bu reklamları artık tıklamayacak). Model, bu tür temel değişikliklere uyum sağlamak için yeniden eğitilmelidir veya tüm işlem hattı, modelin sürekli olarak giriş verisine uyum sağladığı çevrimiçi öğrenme ortamına taşınmalıdır.



**Sade One-Hot Encoding**

* **Bellek gereksinimi**: Seyrek vektör formatında O(n), burada n, veri noktalarının sayısıdır.
* **Hesaplama gereksinimi**: Doğrusal model altında O(nk), burada k, kategori sayısıdır.
* **Avantajlar**:

  * Uygulaması en kolay yöntemdir.
  * Potansiyel olarak en doğru sonuçları verir.
  * Çevrimiçi öğrenme için uygundur.
* **Dezavantajlar**:

  * Hesaplamada verimsizdir.
  * Büyüyen kategorilere uyum sağlamaz.
  * Yalnızca doğrusal modeller için uygundur.
  * Gerçekten büyük veri setlerinde büyük ölçekli dağıtık optimizasyon gerektirir.

**Özellik Hash'leme**

* **Bellek gereksinimi**: Seyrek matris formatında O(n), burada n, veri noktalarının sayısıdır.
* **Hesaplama gereksinimi**: Doğrusal veya çekirdekli model altında O(nm), burada m, hash kutuları sayısıdır.
* **Avantajlar**:

  * Uygulaması kolaydır.
  * Model eğitimini daha ucuz hale getirir.
  * Yeni kategorilere kolayca uyum sağlar.
  * Nadir kategorileri kolayca işler.
  * Çevrimiçi öğrenme için uygundur.
* **Dezavantajlar**:

  * Yalnızca doğrusal veya çekirdekli modeller için uygundur.
  * Hash'lenmiş özellikler yorumlanamaz.
  * Doğrulukla ilgili karışık raporlar vardır.

**Bin Sayma**

* **Bellek gereksinimi**: Küçük, yoğun her veri noktası için O(n+k), her kategori için saklanması gereken sayım istatistikleri ile birlikte.
* **Hesaplama gereksinimi**: Doğrusal modeller için O(n); ayrıca ağaçlar gibi doğrusal olmayan modeller için de kullanılabilir.
* **Avantajlar**:

  * Eğitim sırasında en küçük hesaplama yüküne sahiptir.
  * Ağaç tabanlı modelleri mümkün kılar.
  * Yeni kategorilere uyum sağlaması nispeten kolaydır.
  * **Back-off** veya **count-min sketch** ile nadir kategorileri işler.
  * Yorumlanabilir.
* **Dezavantajlar**:

  * Tarihsel verilere ihtiyaç duyar.
  * Güncellemeler gecikmeli yapılır, çevrimiçi öğrenme için tamamen uygun değildir.
  * Sızıntı olasılığı daha yüksektir.
